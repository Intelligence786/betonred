...
class TaskBloc extends Bloc<TaskEvent, TaskState> {
  ...
  Duration minDuration = Duration(seconds: 5);
  Duration maxDuration = Duration(seconds: 10);
  ...
  void _startTimer() async {
    ...
    void _updateTasks() async {
      if (_activeTasks.length < 5) {
        final newTask = _generateRandomTask();
        _activeTasks.add(newTask);
        add(TaskCheckEvent(newTask.potionModel));
      ...
      await TaskManager.setLastUpdateTime(
          DateTime.now()); // Сохраняем время последнего обновления
    }
  ...
  _onCompleteTask(TaskCompleteEvent event, Emitter<TaskState> emit) async {
    ...
    final taskIndex = _activeTasks.indexWhere((task) => task.potionModel == currentPotion);
    if (taskIndex != -1) {
      ...
      tasksForDate.add(_activeTasks.toList()); // Обновляем данные в tasksForDate
      emit(TaskLoadedState(taskModels: _activeTasks));
    }
  }
}
